...
| P5 | Private / internal network targeting | `S1 SYNTAX` rejects loopback, private (RFC1918), link-local, **CGNAT 100.64/10**, multicast and reserved ranges via `ipaddress`. The API target allow-policy additionally rejects `169.254.169.254` and other cloud metadata endpoints (SSRF defence). |
